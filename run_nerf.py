...
def render(H, W, K, my_c2w=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           near=0., far=1.,
           use_viewdirs=False, c2w_staticcam=None, A=1, M=2,
           **kwargs):
    """Render rays
    Args:
    ...
    if my_c2w is not None:
        rays_o, rays_d = parameterize_rays(H, W, K[0][0], 1., rays_o, rays_d, A, M, my_c2w)
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, depth, acc, _ = render(H, W, K, my_c2w=None, chunk=chunk, c2w=c2w[:3, :4], **render_kwargs)
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    last_dist = torch.full(dists[..., :1].shape, 1e10)
    dists = torch.cat([dists, last_dist], -1)  # [N_rays, N_samples]
    # dists = torch.cat([dists, ([1e10]).extend(dists[...,:1].shape)], -1)  # [N_rays, N_samples]
    # AttributeError: 'list' object has no attribute 'expand'. Did you mean: 'extend'?
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split, bounding_box = load_scannet_data(args.datadir, args.scannet_sceneID,
                                                                                    args.half_res)
        args.bounding_box = bounding_box
    ...
    for i in trange(start, N_iters):
        ...
        rgb, depth, acc, extras = render(H, W, K, my_c2w=None, chunk=args.chunk, rays=batch_rays,
